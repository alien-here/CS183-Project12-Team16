...
#Author:Lin Yinuo
#Function:decode_uploaded_image
...
#Author:Wu Xujing
#Function:clean_text_components
...
#Author:Chen Jiahao
#Function:estimate_skew_by_projection
def v(binary_inverse: np.ndarray) -> float:
    best_angle = 0.0
    best_score = -1.0
...
#Author:Wu Xujing
#Function:estimate_skew_by_hough
...
#Author:Chen Jiahao
#Function:estimate_skew_by_min_area
...
#Author:Chen Jiahao
#Function:rotate_ocr_region
def rotate_ocr_region(region_image: np.ndarray, manual_rotation: float) -> np.ndarray:
    """Rotate the selected OCR area before preview and text-region detection."""
    return rotate_image(region_image, manual_rotation)

#Author:Chen Jiahao
#Function:build_threshold_variants
...
#Author:Chen Jiahao
#Function:format_word_boxes_as_layout
...
#Author:Chen Jiahao
#Function:text_to_plain_paragraphs
...
def preprocess_image(
    ...
    rotated_region = rotate_ocr_region(region_image, manual_rotation)
    cropped, crop_box = crop_text_region(rotated_region)
    ...
    deskewed = rotate_image(sharpened, skew_angle)
    rotated_color = rotate_image(resized, skew_angle)
    ...
    steps = {
        ...
        "Synchronized Rotated OCR Area": cv2.cvtColor(
            rotated_region,
            cv2.COLOR_BGR2RGB,
        ),
    ...
    candidates = {
        "Applied Rotation Original Color": cv2.cvtColor(
            rotated_color,
            cv2.COLOR_BGR2RGB,
        ),
    ...
    if rotation_search_enabled:
        for offset in (-6, 6):
            ...
            rotated = rotate_image(sharpened, skew_angle + offset)
...
#Author:Chen Jiahao
#Function:recognize_text_with_confidence
...
#Author:Chen Jiahao
#Function:recognize_with_easyocr
...
#Author:Chen Zeyue
#Function:render_project_overview
...
#Author:Chen Zeyue
#Function:crop_region_for_preview
...
#Author:Chen Zeyue
#Function:render_rotation_preview
def render_rotation_preview(
    ...
    rotated_region = rotate_ocr_region(selected_region, manual_rotation)
...
#Author:Chen Zeyue
#Function:render_mouse_crop_selector
...
#Author:Chen Zeyue
#Function:render_preprocessing_results
def render_preprocessing_results(result: PreprocessResult) -> None:
    ...
    st.caption(
        ...
        f"{result.region_box} | Rotated-area crop box: {result.crop_box} | "
...
#Author:Chen Zeyue
#Function:render_automation_report
def render_automation_report(result: OCRResult, preprocessing: PreprocessResult) -> None:
    ...
    st.markdown(
        ...
        **Processing Summary:** recognition area `{preprocessing.recognition_area}`,
        region `{preprocessing.region_box}`, rotated-area crop `{preprocessing.crop_box}`,
...
#Author:Chen Zeyue
#Function:main Streamlit OCR application workflow
def main() -> None:
    configure_page()
